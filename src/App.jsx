...
function App() {
  ...
  const pwd_generator=useCallback(()=>{
    ...
    for(let i=1;i<=length;i++){
      let s1=Math.floor(Math.random()*(str.length)+1)
      pass+=str.charAt(s1);
    }
    setPwd(pass);
  ...
  useEffect(()=>{
    console.log("hi ")
    pwd_generator();
  },[length,numberAllowed,charAllowed,pwd_generator])
  ...
  return (
    <>
      ...
      <div className='flex shadow-md rounded-lg overflow-hidden mb-4'>
        ...
        <button onClick={copyPwd}className='outline-none bg-blue-500 text-white px-3 py-0.5 shrink-0'>Copy</button>
      </div>
